...
<?php 
	...
	if( $header_style == 'side' ){
		...
		$header_side_class  = ' arki-style-side';
		$header_side_style = arki_get_option('general', 'header-side-style', 'top-left');

		switch( $header_side_style ){
			case 'top-left': 
				$header_side_class .= ' arki-style-left';
				$body_wrapper_class .= ' arki-left';
				break;
			case 'top-right': 
				$header_side_class .= ' arki-style-right';
				$body_wrapper_class .= ' arki-right';
				break;
			case 'middle-left':
			case 'middle-left-2':
				$header_side_class .= ' arki-style-left arki-style-middle';
				$body_wrapper_class .= ' arki-left';
				break;
			case 'middle-right': 
			case 'middle-right-2': 
				$header_side_class .= ' arki-style-right arki-style-middle';
				$body_wrapper_class .= ' arki-right';
				break;
			default: 
				break;
		...
		$header_side_style = arki_get_option('general', 'header-side-toggle-style', 'left');

		$header_side_class  = ' arki-style-side-toggle';
		$header_side_class .= ' arki-style-' . $header_side_style;
		$body_wrapper_class .= ' arki-' . $header_side_style;

	}else if( $header_style == 'boxed' ){

		$body_wrapper_class .= ' arki-with-transparent-header';
		...
		if( $header_background_style == 'transparent' ){
			if( $header_style == 'plain' ){
				...
				$body_wrapper_class .= ' arki-with-transparent-navigation';
			}
		}
	}
	...
	if( $layout == 'full' && in_array($header_style, array('plain', 'bar', 'boxed')) ){
		$body_wrapper_class .= ' arki-with-frame';
	}
	...
	if( empty($post_option['enable-header-area']) || $post_option['enable-header-area'] == 'enable' ){
		...
		$body_outer_wrapper_class = ' arki-header-disable';
	}
	
	// preload
	$preload = arki_get_option('plugin', 'enable-preload', 'disable');
	if( $preload == 'enable' ){
		echo '<div class="arki-page-preload gdlr-core-page-preload gdlr-core-js" id="arki-page-preload" data-animation-time="500" ></div>';
	}
?>
<div class="arki-body-outer-wrapper <?php echo esc_attr($body_outer_wrapper_class); ?>">
	<?php
		if( empty($post_option['enable-float-social']) ){
			$float_social = arki_get_option('general', 'enable-float-social', 'disable');
		}else{
		...
		if( !empty($post_option['body-background-type']) && $post_option['body-background-type'] == 'image' ){
			echo '<div class="arki-body-background" ' . gdlr_core_esc_style(array(
			...
			$body_wrapper_class .= ' arki-transparent';
		}else{
			$background_type = arki_get_option('general', 'background-type', 'color');
			if( $background_type == 'image' ){
				echo '<div class="arki-body-background" ></div>';
				$body_wrapper_class .= ' arki-transparent';
			}
		}
	?>
	<div class="arki-body-wrapper clearfix <?php echo esc_attr($body_wrapper_class); ?>">
	<?php  
		...
		if( empty($post_option['enable-header-area']) || $post_option['enable-header-area'] == 'enable' ){
			...
			if( $header_style == 'side' || $header_style == 'side-toggle' ){
				...
				echo '<div class="arki-header-side-nav arki-header-background ' . esc_attr($header_side_class) . '" id="arki-header-side-nav" >';

				// header - logo area
				get_template_part('header/header-style', $header_style); 

				echo '</div>';
				echo '<div class="arki-header-side-content ' . esc_attr($header_side_class) . '" >';
				...
				if( $header_style == in_array($header_style, array('plain','bar2')) ){
					if( $header_background_style == 'transparent' ){
						$close_div = true;
						echo '<div class="arki-header-background-transparent" >';
					}
				}else if( $header_style == 'boxed' ){
					$close_div = true;
					echo '<div class="arki-header-boxed-wrap" >';
				}

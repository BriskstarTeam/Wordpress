<?php
	...
	if( arki_get_option('general', 'blog-navigation', 'enable') == 'enable' ){
		...
		if( !empty($prev_post) ){
			$prev_post_html = get_previous_post_link(
				...
				'<span class="arki-text arki-title-font" >' . esc_html__( 'Previous Post', 'arki' ) . '</span>' .
				'%link' . 
				'</span>',
		...
		if( !empty($next_post) ){
			$next_post_html = get_next_post_link( 
				...
				'<span class="arki-text arki-title-font" >' . esc_html__( 'Next Post', 'arki' ) . '</span>' . 
				'%link' . 
				'</span>',
				'<span class="arki-single-nav-title arki-title-font" >%title</span>'
			);
	...
	if( arki_get_option('general', 'blog-social-share', 'enable') == 'enable' ){
		if( class_exists('gdlr_core_pb_element_social_share') ){
			...
			echo '<div class="arki-single-social-share arki-item-rvpdlr clearfix" >';
	...
	// tag 
	echo '<div class="arki-single-magazine-author-tags clearfix" >';
	...
	if( $enable_related_post == 'enable' && class_exists('gdlr_core_blog_style') ){
		...
		$related_post_args = array(
			...
			'blog-column-style' => arki_get_option('general', 'related-post-blog-column-style', 'style-1'),
		...
		if( $query->have_posts() ){
			...
			echo '<div class="arki-single-related-post-wrap arki-item-rvpdlr" >';
			echo '<h3 class="arki-single-related-post-title arki-item-mglr" >' . esc_html__('Related Posts', 'arki') . '</h3>';

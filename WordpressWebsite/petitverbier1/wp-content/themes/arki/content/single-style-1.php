<?php
	...
	if( arki_get_option('general', 'blog-navigation', 'enable') == 'enable' ){
		$prev_post = get_previous_post_link(
			'<span class="arki-single-nav arki-single-nav-left">%link</span>',
			'<i class="arrow_left" ></i><span class="arki-text" >' . esc_html__( 'Prev', 'arki' ) . '</span>'
		);
		$next_post = get_next_post_link(
			'<span class="arki-single-nav arki-single-nav-right">%link</span>',
			'<span class="arki-text" >' . esc_html__( 'Next', 'arki' ) . '</span><i class="arrow_right" ></i>'
		);
		if( !empty($prev_post) || !empty($next_post) ){
			echo '<div class="arki-single-nav-area clearfix" >' . $prev_post . $next_post . '</div>';
		}
	}

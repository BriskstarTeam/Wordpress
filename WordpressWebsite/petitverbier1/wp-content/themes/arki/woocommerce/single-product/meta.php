...
<div class="product_meta arki-title-font">
	...
	<?php if ( wc_product_sku_enabled() && ( $product->get_sku() || $product->is_type( 'variable' ) ) ) : ?>
		...
		<span class="sku_wrapper"><?php esc_html_e( 'SKU:', 'arki' ); ?> <span class="sku"><?php 
			if( $sku = $product->get_sku() ){
				echo gdlr_core_text_filter($sku);
			}else{
			 	echo esc_html__( 'N/A', 'arki' );
			}
		?></span></span>

	<?php endif; ?>

	<?php echo wc_get_product_category_list( $product->get_id(), ', ', '<span class="posted_in">' . _n( 'Category:', 'Categories:', count( $product->get_category_ids() ), 'arki' ) . ' ', '</span>' ); ?>

	<?php echo wc_get_product_tag_list( $product->get_id(), ', ', '<span class="tagged_as">' . _n( 'Tag:', 'Tags:', count( $product->get_tag_ids() ), 'arki' ) . ' ', '</span>' ); ?>
...
<?php
	...
	if( arki_get_option('general', 'blog-social-share', 'enable') == 'enable' ){
		if( class_exists('gdlr_core_pb_element_social_share') ){
			echo '<div class="arki-woocommerce-social-share" >';

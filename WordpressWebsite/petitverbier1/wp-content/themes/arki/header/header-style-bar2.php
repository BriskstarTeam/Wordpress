<?php
	...
	$header_style = arki_get_option('general', 'header-bar2-navigation-align', 'center');
...
?>	
<header class="arki-header-wrap arki-header-style-bar arki-style-2 <?php echo esc_attr($header_wrap_class); ?>" >
	...
	<div class="arki-header-container clearfix <?php echo esc_attr($header_container_class); ?>">
		...
		<?php
			...
			$logo_right_text = '';
			$logo_right_text_content = arki_get_option('general', 'logo-right-text');
			if( !empty($logo_right_text_content) ){
				$logo_right_text .= '<div class="arki-logo-right-text-content" >';
				$logo_right_text .= gdlr_core_content_filter($logo_right_text_content);
				$logo_right_text .= '</div>';
			}

			if( !empty($logo_right_text) ){
				echo '<div class="arki-logo-right-text arki-item-pdlr clearfix" >' . gdlr_core_text_filter($logo_right_text) . '</div>';
			}
		?>
		</div>
	</div>
	<div class="arki-navigation-bar-wrap arki-navigation-header-style-bar arki-style-2 <?php echo esc_attr($navigation_wrap_class); ?>" >
		
		<div class="arki-navigation-container clearfix <?php echo esc_attr($header_container_class); ?>">
			<div class="arki-navigation-background arki-item-mglr" ></div>
			<?php
				$navigation_class = '';
			...
			<?php
				...
				if( has_nav_menu('right_menu') || $enable_search || $enable_cart || $enable_right_button || !empty($custom_main_menu_right) || $side_content_menu ){
					...
					if( $enable_right_button ){
						...
						echo '<a class="arki-main-menu-right-button ' . esc_attr($button_class) . '" href="' . esc_url($button_link) . '" target="' . esc_attr($button_link_target) . '" >';

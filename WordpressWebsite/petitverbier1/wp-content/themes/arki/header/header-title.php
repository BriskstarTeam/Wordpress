<?php
	...
	$title_style = arki_get_option('general', 'default-title-style', 'small');
	$title_align = arki_get_option('general', 'default-title-align', 'left');

	// header gradient
	$header_gradient = arki_get_option('general', 'page-title-top-bottom-gradient', 'none');

	// for index page
	if( is_home() ){
		$main_head = true;
		$page_title = esc_html__('Blog', 'arki');
	...
	// tribe event
		...
		$page_title = get_the_title($arki_event_id);

		$events_label_plural = tribe_get_event_label_plural();
		$event_link  = '<div class="arki-page-title-event-link" >';
		$event_link .= '<a href="' . esc_url(tribe_get_events_link()) . '">';
		$event_link .= sprintf('&laquo; ' . esc_html_x('All %s', '%s Events plural label', 'arki'), $events_label_plural);
		$event_link .= '</a>';
		$event_link .= '</div>';

		$event_time = tribe_events_event_schedule_details($arki_event_id, '', '');
		if( tribe_get_cost($arki_event_id) ){
			$event_time .= '<span class="tribe-events-cost">' . tribe_get_cost($arki_event_id, true) . '</span>';
		}
		if( !empty($event_time) ){
			$event_time = '<div class="arki-page-title-event-time" >' . $event_time . '</div>';
		}
	...
	// for pages
		...
		if( empty($post_option['enable-breadcrumbs']) || $post_option['enable-breadcrumbs'] == 'default' ){
			$breadcrumbs = arki_get_option('general', 'enable-breadcrumbs', 'disable');
		}else{
	...
	// search page
		...
		if( have_posts() ){
			$page_title = esc_html__('Search Results', 'arki');
	...
	// archive page
		...
		if( !class_exists('gdlr_core_pb_element_product') && 
			...
			$page_title = esc_html__('Category', 'arki');
			...
			$page_title = esc_html__('Tag', 'arki');
	...
	if( (empty($post_option['enable-page-title']) || $post_option['enable-page-title'] == 'enable') && (!empty($page_title) || !empty($page_caption)) ){	
		...
		$page_title = apply_filters('arki_page_title', $page_title);
		$page_caption = apply_filters('arki_page_caption', $page_caption);

		$extra_class  = ' arki-style-' . $title_style;
		$extra_class .= ' arki-' . $title_align . '-align';

		echo '<div class="arki-page-title-wrap ' . esc_attr($extra_class) . '" ' . gdlr_core_esc_style(array(
			'background-image' => empty($title_background)? '': $title_background
		)) . '>';
		echo '<div class="arki-header-transparent-substitute" ></div>';
		echo '<div class="arki-page-title-overlay" ' . gdlr_core_esc_style(array(
		...
		if( $header_gradient == 'top' || $header_gradient == 'both' ){
			echo '<div class="arki-page-title-top-gradient" ></div>';
		}
		if( $header_gradient == 'bottom' || $header_gradient == 'both' ){
			echo '<div class="arki-page-title-bottom-gradient" ></div>';
		}
		echo '<div class="arki-page-title-container arki-container" >';
		echo '<div class="arki-page-title-content arki-item-pdlr" ' . gdlr_core_esc_style(array(
		...
		if( !empty($page_title) ){
			if( !empty($main_head) ){
				echo '<h1 class="arki-page-title" ' . gdlr_core_esc_style(array(
				...
				echo '<h3 class="arki-page-title" ' . gdlr_core_esc_style(array(
					'font-size' => empty($title_size['title-size'])? '': $title_size['title-size'],
					'font-weight' => empty($title_weight['title-weight'])? '': $title_weight['title-weight'],
					'text-transform' => (empty($title_transform) || $title_transform == 'default')? '': $title_transform,
					'letter-spacing' => empty($title_size['title-letter-spacing'])? '': $title_size['title-letter-spacing'],
		...
		if( !empty($page_caption) ){
			echo '<div class="arki-page-caption" ' . gdlr_core_esc_style(array(
		...
		if( function_exists('bcn_display') && $breadcrumbs == 'enable' ){
			...
			echo '<div class="arki-breadcrumbs" ' . gdlr_core_esc_style(array(

  ...
  class PluginClass extends \PuvoxSoftware\default_plugin
  {
	...
	public function change_permalinks( $permalink, $post=false, $leavename=false ) { 
		...
		foreach ($postTypes  as $each_post_type){
			if($post->post_type == $each_post_type){
				...
				$permalink = $this->helpers->remove_extra_slashes('/'. $this->helpers->get_parents_slugpath($post). '/'. '%postname%' );
			}
		}
		return $permalink;
	}
	...
	{ 
		...
		{ 
			...
			<script>
			ttLibrary.radiobox_onchange_hider('input[name=add-hierarchy-parent-to-post\\[hierarchy_permalinks_too\\]]',  '0',  '.hierarchyMethodDesc');
			</script>
		<?php 
		}

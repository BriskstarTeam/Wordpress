        ...
        switch($column_name) {
            ...
            case 'links':
                ...
                foreach ($links as $link) {
                    $list .= '<li><i data-post_id="'.$link->post->id.'" data-post_type="'.$link->post->type.'" data-anchor="" data-url="'.base64_encode($link->url).'" class="wpil_link_delete dashicons dashicons-no-alt"></i><div><a href="' . $link->url . '" target="_blank">' . $link->url . '</a><br><a href="' . $link->post->getLinks()->view . '" target="_blank"><b>[' . $link->anchor . ']</b></a></div></li>';
                }
                $list .= '</ul>';
    ...
    function extra_tablenav( $which ) {
        if ($which == "bottom") {
            ...
            <div class="alignright actions bulkactions detailed_export">
                <a href="javascript:void(0)" class="button-primary csv_button" data-type="domains" id="wpil_cvs_export_button">Detailed Export to CSV</a>
            </div>
            <?php
        }
    }

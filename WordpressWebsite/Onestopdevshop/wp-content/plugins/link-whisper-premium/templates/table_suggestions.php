<table class="wp-list-table widefat fixed striped posts tbl_keywords_x js-table wpil-outbound-links" id="tbl_keywords">
    <?php if (!empty($phrases)) : ?>
        ...
        <tr>
            <th>
                <div>
                    <b>Add Internal Outbound Links</b>
                    <br />
                    <div style="margin:5px 0 0 0;">
                        <input type="checkbox" id="select_all"><span style="margin:0 0 0 5px; font-weight:300">Check All</span>
                    </div>
                </div>
            </th>
            <th>
                ...
                <td class="sentences">
                    <?php foreach ($phrase->suggestions as $suggestion) : ?>
                        <div class="sentence" data-id="<?=esc_attr($suggestion->post->id)?>" data-type="<?=esc_attr($suggestion->post->type)?>">
                            ...
                            <span class="wpil_sentence_with_anchor"><?=$suggestion->sentence_with_anchor?></span>
                ...
                <td>
                    <?php if (count($phrase->suggestions) > 1) : ?>
                        <div class="wpil-collapsible-wrapper">
                            <div class="wpil-collapsible wpil-collapsible-static wpil-links-count">
                                <div style="opacity:<?=$phrase->suggestions[0]->opacity?>" data-id="<?=esc_attr($phrase->suggestions[0]->post->id)?>" data-type="<?=esc_attr($phrase->suggestions[0]->post->type)?>">
                                    <?=esc_attr($phrase->suggestions[0]->post->getTitle())?>
                                    <?=!empty(Wpil_Suggestion::$undeletable)?' ('.esc_attr($phrase->suggestions[0]->post_score).')':''?>
                                    <br>
                                    <a class="post-slug" target="_blank" href="<?=$phrase->suggestions[0]->post->getLinks()->view?>">/<?=$phrase->suggestions[0]->post->getSlug()?></a>
                                        ...
                                        <li>
                                            <div>
                                                ...
                                                <span class="data">
                                                    ...
                                                    <br>
                                                    <a class="post-slug" target="_blank" href="<?=$suggestion->post->getLinks()->view?>">/<?=$suggestion->post->getSlug()?></a>
                                                </span>
                                            </div>
                                        </li>
                                    <?php endforeach; ?>
                    ...
                    <?php else : ?>
                        <div style="opacity:<?=$phrase->suggestions[0]->opacity?>" class="suggestion" data-id="<?=esc_attr($phrase->suggestions[0]->post->id)?>" data-type="<?=esc_attr($phrase->suggestions[0]->post->type)?>">
                            ...
                            <?=!empty(Wpil_Suggestion::$undeletable)?' ('.esc_attr($phrase->suggestions[0]->post_score).')':''?>
                            <br>
                            <a class="post-slug" target="_blank" href="<?=$phrase->suggestions[0]->post->getLinks()->view?>">

...
if ( !function_exists('happy_baby_need_page_title') ) {
	function happy_baby_need_page_title() {
		return !is_front_page() && apply_filters('happy_baby_filter_need_page_title', true);
	}
}
...
if ( !function_exists('happy_baby_show_layout') ) {
	function happy_baby_show_layout($str, $before='', $after='') {
		if (trim($str) != '') {
...
if ( !function_exists('happy_baby_get_logo_image') ) {
	function happy_baby_get_logo_image($type='') {
		$logo_image = '';
		if (happy_baby_get_retina_multiplier(2) > 1)
			$logo_image = happy_baby_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '').'_retina' );
		if (empty($logo_image)) 
			$logo_image = happy_baby_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '') );
		return $logo_image;
	}
}

// Return header video (if set)
if ( !function_exists('happy_baby_get_header_video') ) {
	function happy_baby_get_header_video() {
		$video = '';
		if (apply_filters('happy_baby_header_video_enable', !wp_is_mobile() && is_front_page())) {
			if (happy_baby_check_theme_option('header_video')) {
				$video = happy_baby_get_theme_option('header_video');
...
if ( !function_exists('happy_baby_show_post_meta') ) {
	function happy_baby_show_post_meta($args=array()) {
		...
		?><div class="post_meta"><?php
			...
			foreach ($components as $comp) {
				...
				// Post categories
				if ($comp == 'categories') {
					$cats = get_post_type()=='post' ? get_the_category_list(', ') : apply_filters('happy_baby_filter_get_post_categories', '');
					if (!empty($cats)) {
						?>
						<span class="post_meta_item post_categories"><?php happy_baby_show_layout($cats); ?></span>
						<?php
					}

				// Post date
				} else if ($comp == 'date') {
					$dt = apply_filters('happy_baby_filter_get_post_date', happy_baby_get_date());
					if (!empty($dt)) {
						?>
				...
				// Post author
					...
					if ($author_id > 0) {
						...
						<a class="post_meta_item post_author" rel="author" href="<?php echo esc_url($author_link); ?>">
							<?php echo esc_html__('by ', 'happy-baby'). esc_html($author_name); ?>
						</a>
						<?php
					}
				...
				// Post counters
					...
					happy_baby_show_layout(happy_baby_get_post_counters($args['counters']));
	
				// Socials share
				} else if ($comp == 'share') {
					happy_baby_show_share_links(array(
							'type' => 'drop',
							'caption' => esc_html__('Share', 'happy-baby'),
				...
				// Edit page link
					...
					edit_post_link( esc_html__( 'Edit', 'happy-baby' ), '<span class="post_meta_item post_edit">', '</span>' );
				}
			}
		?></div><!-- .post_meta --><?php
...
if ( !function_exists('happy_baby_show_post_featured') ) {
	function happy_baby_show_post_featured($args=array()) {

		$args = array_merge(array(
			'hover' => happy_baby_get_theme_option('image_hover'),	// Hover effect
			...
			'seo' => happy_baby_is_on(happy_baby_get_theme_option('seo_snippets')),
			'singular' => is_singular()						// Current page is singular (true) or blog/shortcode (false)
			), $args);

		if ( post_password_required() ) return;

		$thumb_size = !empty($args['thumb_size']) ? $args['thumb_size'] : happy_baby_get_thumb_size(is_attachment() ? 'full' : (is_single() ? 'huge' : 'big'));
		$post_format = str_replace('post-format-', '', get_post_format());
		$no_image = !empty($args['show_no_image']) ? happy_baby_get_no_image() : '';
		if ($args['thumb_bg']) {
			if (has_post_thumbnail()) {
				$image = wp_get_attachment_image_src( get_post_thumbnail_id(get_the_ID()), $thumb_size );
				$image = $image[0];
			} else if ($post_format == 'image') {
				$image = happy_baby_get_post_image();
				if (!empty($image)) 
					$image = happy_baby_add_thumb_size($image, $thumb_size);
			}
			if (empty($image))
				$image = $no_image;
			if (!empty($image))
				$args['class'] .= ($args['class'] ? ' ' : '') . 'post_featured_bg' . ' ' . happy_baby_add_inline_css_class('background-image: url('.esc_url($image).');');
		}
			...
			if ( is_attachment() ) {
				...
				<div class="post_featured<?php if ($args['class']) echo ' '.esc_attr($args['class']); ?>"<?php if ($args['seo']) echo ' itemscope itemprop="image" itemtype="//schema.org/ImageObject"'; ?>>
					...
					if (has_post_thumbnail() && $args['seo']) {
						$happy_baby_attr = happy_baby_getimagesize( wp_get_attachment_url( get_post_thumbnail_id() ) );
						?>
						<meta itemprop="width" content="<?php echo esc_attr($happy_baby_attr[0]); ?>">
						<meta itemprop="height" content="<?php echo esc_attr($happy_baby_attr[1]); ?>">
						<?php
					}
					if (!$args['thumb_bg']) {
						if ( has_post_thumbnail() ) {
			...
			if ($has_thumb 
				...
				if ( $args['thumb_bg'] ) {
					...
					if (!in_array($post_format, array('audio', 'video'))) {
						happy_baby_hovers_add_icons($args['hover']);
					}
					...
					if (($output=happy_baby_get_slider_layout(array('thumb_size'=>$thumb_size, 'controls'=>'yes', 'pagination'=>'yes'))) != '')
						happy_baby_show_layout($output);
					...
					if (!empty($image)) {
						...
						happy_baby_hovers_add_icons($args['hover'], array('image' => $image));
					}
				...
				if (!$args['thumb_only'] && ($post_format == 'video' || $post_format == 'audio')) {
					...
					$post_content = happy_baby_get_post_content();
					$post_content_parsed = $post_content;
					$video = happy_baby_get_post_video( $post_content, false );
						if ( empty( $video ) ) {
							$video = happy_baby_get_post_iframe( $post_content, false );
						}
						if ( empty( $video ) ) {
							// Only get video from the content if a playlist isn't present.
							$post_content_parsed = happy_baby_filter_post_content( $post_content );
							if ( false === strpos( $post_content_parsed, 'wp-playlist-script' ) ) {
								...
								if ( ! empty( $videos ) && is_array( $videos ) ) {
									$video = happy_baby_array_get_first( $videos, false );
								}
							}
						}
						if ( ! empty( $video ) ) {
							if ( $has_thumb ) {
								$video = happy_baby_make_video_autoplay( $video );
								?>
					...
					// Put audio over the thumb
						...
						$audio = happy_baby_get_post_audio($post_content, false);
						if (empty($audio))
							$audio = happy_baby_get_post_iframe($post_content, false);
						if (!empty($audio)) {
							//Show metadata (for the future version)
							if (false && function_exists('wp_read_audio_metadata')) {
								$src = happy_baby_get_post_audio($audio);
							...
							?><div class="post_audio<?php if (strpos($audio, 'soundcloud')!==false) echo ' with_iframe'; ?>"><?php
								...
								$media_author = happy_baby_get_theme_option('media_author', '', false, get_the_ID());
								$media_title = happy_baby_get_theme_option('media_title', '', false, get_the_ID());
								if ( !empty($media_author) && !happy_baby_is_inherit($media_author) ) {
									?><div class="post_audio_author"><?php happy_baby_show_layout($media_author); ?></div><?php
								}
								if ( !empty($media_title) && !happy_baby_is_inherit($media_title) ) {
									?><h5 class="post_audio_title"><?php happy_baby_show_layout($media_title); ?></h5><?php
								}
								// Display audio
								happy_baby_show_layout($audio);
							?></div><?php
						}
					}
				}
				...
				happy_baby_show_layout($post_info);
				?></div><?php
			} else {
				// Put optional info block over the thumb
				happy_baby_show_layout($post_info);
...
if ( !function_exists('happy_baby_get_no_image') ) {
	function happy_baby_get_no_image($no_image='') {
		static $img = '';
		if (empty($img)) {
			$img = happy_baby_get_theme_option( 'no_image' );
			if (empty($img)) $img = happy_baby_get_file_url('images/no-image.jpg');
		}
...
if ( !function_exists('happy_baby_add_bg_in_post_nav') ) {
	function happy_baby_add_bg_in_post_nav() {
		if ( ! is_single() ) return;
	
		$previous = ( is_attachment() ) ? get_post( get_post()->post_parent ) : get_adjacent_post( false, '', true );
		$next     = get_adjacent_post( false, '', false );
		$css      = '';
		$noimg    = happy_baby_get_no_image();
		
		if ( is_attachment() && $previous->post_type == 'attachment' ) return;
	
		if ( $previous ) {
			if ( has_post_thumbnail( $previous->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $previous->ID ), happy_baby_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = $noimg;
			if ( !empty($img) )
		...
		if ( $next ) {
			if ( has_post_thumbnail( $next->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $next->ID ), happy_baby_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = $noimg;
			if ( !empty($img) )
		...
		wp_add_inline_style( 'happy-baby-main', $css );
	}
}

// Show related posts
if ( !function_exists('happy_baby_show_related_posts') ) {
	function happy_baby_show_related_posts($args=array(), $style=1, $title='') {
		$args = array_merge(array(
		...
		$taxonomy = $args['post_type'] == 'post' ? 'category' : happy_baby_get_post_type_taxonomy();
			...
			<section class="related_wrap">
				<h3 class="section_title related_wrap_title"><?php
					if (!empty($title))
						...
						esc_html_e('You May Also Like', 'happy-baby');
				?></h3>
...
if ( !function_exists('happy_baby_show_portfolio_posts') ) {
	function happy_baby_show_portfolio_posts($args=array()) {
		$args = array_merge(array(
			'cat' => 0,
			'parent_cat' => 0,
			'taxonomy' => 'category',
		...
		$blog_style = explode('_', empty($args['blog_style']) ? happy_baby_get_theme_option('blog_style') : $args['blog_style']);
		$style = $blog_style[0];
		...
		if ( !$args['echo'] ) {
			...
			$q_args = happy_baby_query_add_posts_and_cats($q_args, '', $args['post_type'], $args['cat'], $args['taxonomy']);
			if ($args['page'] > 1) {
				$q_args['paged'] = $args['page'];
				$q_args['ignore_sticky_posts'] = true;
			}
			$ppp = happy_baby_get_theme_option('posts_per_page');
			if ((int) $ppp != 0)
...
// AJAX handler for the happy_baby_ajax_get_posts action
if ( !function_exists( 'happy_baby_ajax_get_posts_callback' ) ) {
	add_action('wp_ajax_happy_baby_ajax_get_posts',			'happy_baby_ajax_get_posts_callback');
	add_action('wp_ajax_nopriv_happy_baby_ajax_get_posts',	'happy_baby_ajax_get_posts_callback');
	function happy_baby_ajax_get_posts_callback() {
		if ( !wp_verify_nonce( happy_baby_get_value_gp('nonce'), admin_url('admin-ajax.php') ) )
			wp_die();
	
		$id = !empty($_REQUEST['blog_template']) ? happy_baby_get_value_gpc('blog_template') : 0;
		if ((int) $id > 0) {
			happy_baby_storage_set('blog_archive', true);
			happy_baby_storage_set('blog_mode', 'blog');
			happy_baby_storage_set('options_meta', get_post_meta($id, 'happy_baby_options', true));
		}

		$response = array(
			'error'=>'', 
			'data' => happy_baby_show_portfolio_posts(array(
							'cat' => (int) happy_baby_get_value_gpc('cat'),
							'parent_cat' => (int) happy_baby_get_value_gpc('parent_cat'),
							'page' => (int) happy_baby_get_value_gpc('page'),
							'post_type' => trim(happy_baby_get_value_gpc('post_type')),
							'taxonomy' => trim(happy_baby_get_value_gpc('taxonomy')),
							'blog_style' => trim(happy_baby_get_value_gpc('blog_style')),
							'echo' => false
							)
						)
		);
		...
		if (empty($response['data'])) {
			$response['error'] = esc_html__('Sorry, but nothing matched your search criteria.', 'happy-baby');
		}
...
// Show pagination
if ( !function_exists('happy_baby_show_pagination') ) {
	function happy_baby_show_pagination() {
		global $wp_query;
		// Pagination
		$pagination = happy_baby_get_theme_option('blog_pagination');
		if ($pagination == 'pages') {
			the_posts_pagination( array(
				'mid_size'  => 2,
				'prev_text' => esc_html__( '<', 'happy-baby' ),
				'next_text' => esc_html__( '>', 'happy-baby' ),
				'before_page_number' => '<span class="meta-nav screen-reader-text">' . esc_html__( 'Page', 'happy-baby' ) . ' </span>',
			) );
			...
			if ($page_number < $wp_query->max_num_pages) {
				...
				<div class="nav-links-more<?php if ($pagination == 'infinite') echo ' nav-links-infinite'; ?>">
					<a class="nav-load-more" href="#" 
						...
						><span><?php esc_html_e('Load more posts', 'happy-baby'); ?></span></a>
				</div>
				<?php
			}
			...
			<div class="nav-links-old">
				<span class="nav-prev"><?php previous_posts_link( is_search() ? esc_html__('Previous posts', 'happy-baby') : esc_html__('Newest posts', 'happy-baby') ); ?></span>
				<span class="nav-next"><?php next_posts_link( is_search() ? esc_html__('Next posts', 'happy-baby') : esc_html__('Older posts', 'happy-baby'), $wp_query->max_num_pages ); ?></span>
			</div>
			<?php
		}
	}
...
if ( ! function_exists( 'happy_baby_single_comments_field' ) ) {
    function happy_baby_single_comments_field( $args ) {
        ...
        if ( 'text' == $args['field_type'] ) {
            ...
            $html .= '<input id="' . esc_attr( $args['field_name'] ) . '" name="' . esc_attr( $args['field_name'] ) . '" type="checkbox" value="' . esc_attr( $args['field_value'] ) . '"' . ( $args['field_req'] ? ' aria-required="true"' : '' ) . ' />'
                    . ' <label for="' . esc_attr( $args['field_name'] ) . '" class="' . esc_attr( $args['field_req'] ? 'required' : 'optional' ) . '">' . wp_kses( $args['field_title'], 'happy_baby_kses_content' ) . '</label>';
        } else {
        ...
        if ( 'default' != $args['form_style'] ) {
            $html .= '<span class="sc_form_field_hover">'
                        ...
                        . '<span class="sc_form_field_content" data-content="' . esc_attr( $args['field_title'] ) . '">' . wp_kses( $args['field_title'], 'happy_baby_kses_content' ) . '</span>'
                    . '</span>';
        }
        $html .= '</span></div>';
        return $html;

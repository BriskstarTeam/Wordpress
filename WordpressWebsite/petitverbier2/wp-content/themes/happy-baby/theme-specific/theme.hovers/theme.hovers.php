...
if (!function_exists('happy_baby_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'happy_baby_hovers_theme_setup3', 3 );
	function happy_baby_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		happy_baby_storage_set_array_after('options', 'border_radius', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'happy-baby'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'happy-baby') ),
				"std" => 'default',
				"options" => array(
					'default'		=> esc_html__('Fade',				'happy-baby')
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'happy-baby'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'happy-baby') ),
				"std" => 'icon',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'dots'	=> esc_html__('Dots',	'happy-baby'),
					'icon'	=> esc_html__('Icon',	'happy-baby'),
					'icons'	=> esc_html__('Icons',	'happy-baby'),
					'zoom'	=> esc_html__('Zoom',	'happy-baby'),
					'fade'	=> esc_html__('Fade',	'happy-baby'),
					'slide'	=> esc_html__('Slide',	'happy-baby'),
					'pull'	=> esc_html__('Pull',	'happy-baby'),
					'border'=> esc_html__('Border',	'happy-baby')
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('happy_baby_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'happy_baby_hovers_theme_setup9', 9 );
	function happy_baby_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',		'happy_baby_hovers_frontend_scripts', 1010 );
		add_filter( 'happy_baby_filter_localize_script','happy_baby_hovers_localize_script' );
		add_filter( 'happy_baby_filter_merge_scripts',	'happy_baby_hovers_merge_scripts' );
		add_filter( 'happy_baby_filter_merge_styles',	'happy_baby_hovers_merge_styles' );
		add_filter( 'happy_baby_filter_get_css', 		'happy_baby_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'happy_baby_hovers_frontend_scripts' ) ) {
	
	function happy_baby_hovers_frontend_scripts() {
		if ( happy_baby_is_on(happy_baby_get_theme_option('debug_mode')) && happy_baby_get_file_dir('theme-specific/theme.hovers/theme.hovers.js')!='' )
			wp_enqueue_script( 'happy-baby-hovers', happy_baby_get_file_url('theme-specific/theme.hovers/theme.hovers.js'), array('jquery'), null, true );
		if ( happy_baby_is_on(happy_baby_get_theme_option('debug_mode')) && happy_baby_get_file_dir('theme-specific/theme.hovers/theme.hovers.css')!='' )
			wp_enqueue_style( 'happy-baby-hovers',  happy_baby_get_file_url('theme-specific/theme.hovers/theme.hovers.css'), array(), null );
	}
}

// Merge hover effects into single js
if (!function_exists('happy_baby_hovers_merge_scripts')) {
	
	function happy_baby_hovers_merge_scripts($list) {
...
if (!function_exists('happy_baby_hovers_merge_styles')) {
	
	function happy_baby_hovers_merge_styles($list) {
...
if (!function_exists('happy_baby_hovers_localize_script')) {
	
	function happy_baby_hovers_localize_script($arr) {
		$arr['button_hover'] = happy_baby_get_theme_option('button_hover');
		return $arr;
	}
}

// Add hover icons on the featured image
if ( !function_exists('happy_baby_hovers_add_icons') ) {
	function happy_baby_hovers_add_icons($hover, $args=array()) {
		...
		// Hover style 'Shop'
			...
			<div class="icons">
				<?php
				if (false && !is_object($args['cat']) && $product->is_purchasable() && $product->is_in_stock()) {
					echo apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? esc_html__('Buy now', 'happy-baby') : '')
										. '</a>',
										$product );
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink()); ?>" aria-hidden="true" class="icon-visibility"><?php
					if ($hover == 'shop_buttons') {
						if (is_object($args['cat']))
							esc_html_e('View products', 'happy-baby');
						else
							esc_html_e('Details', 'happy-baby');
					}
				?></a>
			</div>
			<?php
		...
		// Hover style 'Icon'
			...
			?><div class="icons"><a href="<?php the_permalink(); ?>" aria-hidden="true" class="icon-visibility"></a></div><?php
...
// Add styles into CSS
if ( !function_exists( 'happy_baby_hovers_get_css' ) ) {
	
	function happy_baby_hovers_get_css($css, $colors, $fonts, $scheme='') {

...
------------------------------------------------------------------------------- */
if (!function_exists('happy_baby_trx_addons_theme_specific_setup1')) {
	add_filter( 'trx_addons_filter_components_editor', 'happy_baby_trx_addons_theme_specific_components');
	function happy_baby_trx_addons_theme_specific_components($enable=false) {
		return false;
	}
}

if (!function_exists('happy_baby_trx_addons_theme_specific_setup1')) {
	add_action( 'after_setup_theme', 'happy_baby_trx_addons_theme_specific_setup1', 1 );
	add_action( 'trx_addons_action_save_options', 'happy_baby_trx_addons_theme_specific_setup1', 1 );
	function happy_baby_trx_addons_theme_specific_setup1() {
		if (happy_baby_exists_trx_addons()) {
			add_filter( 'trx_addons_cv_enable',					'happy_baby_trx_addons_cv_enable');
			add_filter( 'trx_addons_demo_enable',				'happy_baby_trx_addons_demo_enable');
			add_filter( 'trx_addons_filter_edd_themes_market',	'happy_baby_trx_addons_edd_themes_market_enable');
			add_filter( 'trx_addons_cpt_list',					'happy_baby_trx_addons_cpt_list');
			add_filter( 'trx_addons_sc_list',					'happy_baby_trx_addons_sc_list');
			add_filter( 'trx_addons_widgets_list',				'happy_baby_trx_addons_widgets_list');
		}
	}
}

// CV
if ( !function_exists( 'happy_baby_trx_addons_cv_enable' ) ) {
	
	function happy_baby_trx_addons_cv_enable($enable=false) {
...
// Demo mode
if ( !function_exists( 'happy_baby_trx_addons_demo_enable' ) ) {
	
	function happy_baby_trx_addons_demo_enable($enable=false) {
		// To do: return false if theme not use CV functionality
		return true;
	}
}
...
// EDD Themes market
if ( !function_exists( 'happy_baby_trx_addons_edd_themes_market_enable' ) ) {
	
	function happy_baby_trx_addons_edd_themes_market_enable($enable=false) {
...
// CPT
if ( !function_exists( 'happy_baby_trx_addons_cpt_list' ) ) {
	
	function happy_baby_trx_addons_cpt_list($list=array()) {
...
// Shortcodes
if ( !function_exists( 'happy_baby_trx_addons_sc_list' ) ) {
	
	function happy_baby_trx_addons_sc_list($list=array()) {
...
// Widgets
if ( !function_exists( 'happy_baby_trx_addons_widgets_list' ) ) {
	
	function happy_baby_trx_addons_widgets_list($list=array()) {
...
if ( !function_exists( 'happy_baby_trx_addons_menu_cache' ) ) {
	add_filter( 'trx_addons_filter_menu_cache', 'happy_baby_trx_addons_menu_cache');
	function happy_baby_trx_addons_menu_cache($list=array()) {
...
if (!function_exists('happy_baby_trx_addons_localize_script')) {
	...
	function happy_baby_trx_addons_localize_script($arr) {
		$arr['alter_link_color'] = happy_baby_get_scheme_color('alter_link');
		return $arr;
	}
}
...
if ( !function_exists( 'happy_baby_trx_addons_sc_type' ) ) {
	add_filter( 'trx_addons_sc_type', 'happy_baby_trx_addons_sc_type', 10, 2);
	function happy_baby_trx_addons_sc_type($list, $sc) {
...
if ( !function_exists( 'happy_baby_trx_addons_sc_atts' ) ) {
	add_filter( 'trx_addons_sc_atts', 'happy_baby_trx_addons_sc_atts', 10, 2);
	function happy_baby_trx_addons_sc_atts($atts, $sc) {
...
if ( !function_exists( 'happy_baby_trx_addons_sc_map' ) ) {
	add_filter( 'trx_addons_sc_map', 'happy_baby_trx_addons_sc_map', 10, 2);
	function happy_baby_trx_addons_sc_map($params, $sc) {
		...
		$param = array(
			...
			'heading' => esc_html__('Color style', 'happy-baby'),
			'description' => wp_kses_data( __('Select color style to decorate this block', 'happy-baby') ),
			'edit_field_class' => 'vc_col-sm-4',
			'admin_label' => true,
			'value' => array(
				esc_html__('Default', 'happy-baby') => 'default',
				esc_html__('Link 2', 'happy-baby') => 'link2',
				esc_html__('Link 3', 'happy-baby') => 'link3',
				esc_html__('Dark', 'happy-baby') => 'dark'
			),
			'type' => 'dropdown'
		);
...
if ( !function_exists( 'happy_baby_trx_addons_sc_output' ) ) {
	add_filter( 'trx_addons_sc_output', 'happy_baby_trx_addons_sc_output', 10, 4);
	function happy_baby_trx_addons_sc_output($output, $sc, $atts, $content) {
		...
		if (in_array($sc, array('trx_sc_action'))) {
			...
			if (!empty($atts['color_style']) && !happy_baby_is_inherit($atts['color_style']))
...
if ( !function_exists( 'happy_baby_trx_addons_sc_item_title_tag' ) ) {
	add_filter( 'trx_addons_filter_sc_item_title_tag', 'happy_baby_trx_addons_sc_item_title_tag');
	function happy_baby_trx_addons_sc_item_title_tag($tag='') {
		return $tag=='h1' ? 'h2' : $tag;
	}
}

// Return args for the item's button
if ( !function_exists( 'happy_baby_trx_addons_sc_item_button_args' ) ) {
	add_filter( 'trx_addons_filter_sc_item_button_args', 'happy_baby_trx_addons_sc_item_button_args', 10, 3);
	function happy_baby_trx_addons_sc_item_button_args($args, $sc, $sc_args) {
...
if ( !function_exists( 'happy_baby_trx_addons_slider_title' ) ) {
	add_filter( 'trx_addons_filter_slider_title',	'happy_baby_trx_addons_slider_title', 10, 2 );
	function happy_baby_trx_addons_slider_title($title, $data) {
		$title = '';
		if (!empty($data['title'])) 
...
if ( !function_exists( 'happy_baby_trx_addons_sc_googlemap_styles' ) ) {
	add_filter( 'trx_addons_filter_sc_googlemap_styles',	'happy_baby_trx_addons_sc_googlemap_styles');
	function happy_baby_trx_addons_sc_googlemap_styles($list) {
		$list[esc_html__('Dark', 'happy-baby')] = 'dark';
		$list[esc_html__('Extra', 'happy-baby')] = 'extra';
		return $list;
	}
}
...
if ( !function_exists( 'happy_baby_trx_addons_editor_init' ) ) {
	if (is_admin()) add_filter( 'tiny_mce_before_init', 'happy_baby_trx_addons_editor_init', 11);
	function happy_baby_trx_addons_editor_init($opt) {
		if (happy_baby_exists_trx_addons()) {
			...
			if (false && !empty($opt['style_formats'])) {
				...
				if (is_array($style_formats) && count($style_formats)>0 ) {
					foreach ($style_formats as $k=>$v) {
						if ( $v['title'] == esc_html__('List styles', 'happy-baby') ) {
							$style_formats[$k]['items'][] = array(
										'title' => esc_html__('Arrow', 'happy-baby'),
										'selector' => 'ul',
...
if ( !function_exists( 'happy_baby_trx_addons_allow_override_header_image' ) ) {
	add_filter( 'happy_baby_filter_allow_override_header_image', 'happy_baby_trx_addons_allow_override_header_image' );
	function happy_baby_trx_addons_allow_override_header_image($allow) {
		return happy_baby_is_team_page() || happy_baby_is_portfolio_page() ? false : $allow;
	}
}

// Hide sidebar on the team and portfolio pages
if ( !function_exists( 'happy_baby_trx_addons_sidebar_present' ) ) {
	add_filter( 'happy_baby_filter_sidebar_present', 'happy_baby_trx_addons_sidebar_present' );
	function happy_baby_trx_addons_sidebar_present($present) {
		return !is_single() && (happy_baby_is_team_page() || happy_baby_is_portfolio_page()) ? false : $present;
	}
}

// Get thumb size for the team items
if ( !function_exists( 'happy_baby_trx_addons_thumb_size' ) ) {
	add_filter( 'trx_addons_filter_thumb_size',	'happy_baby_trx_addons_thumb_size', 10, 2);
	function happy_baby_trx_addons_thumb_size($thumb_size='', $type='') {
		if ($type == 'team-default')
			$thumb_size = happy_baby_get_thumb_size('cub');
		return $thumb_size;
	}
}
...
if (!function_exists('happy_baby_trx_addons_override_fields')) {
	add_filter( 'trx_addons_filter_override_options_fields', 'happy_baby_trx_addons_override_fields', 10, 2);
	function happy_baby_trx_addons_override_fields($mb, $post_type) {
		if (defined('TRX_ADDONS_CPT_TEAM_PT') && $post_type==TRX_ADDONS_CPT_TEAM_PT) {
			$mb['email'] = array(
				"title" => esc_html__("E-mail",  'happy-baby'),
				"desc" => wp_kses_data( __("Team member's email", 'happy-baby') ),
				"std" => "",
				"details" => true,
				"type" => "text"
			);

...
jQuery(document).ready(function() {
	...
	function happy_baby_ready_actions() {
		...
		jQuery('select:not(.esg-sorting-select):not(#calc_shipping_country):not([class*="trx_addons_attrib_"])').each(function() {
			var s = jQuery(this);
	...
	function happy_baby_init_media_elements(cont) {
		if (HAPPY_BABY_STORAGE['use_mediaelements'] && cont.find('audio:not(.inited),video:not(.inited)').length > 0) {
			if (window.mejs) {
